...
Validate `marketplace.json` with tiered-severity checks:

```bash
python3 scripts/marketplace_manager.py validate <marketplace-root> --json
```

Errors (exit 1) block install: malformed JSON, missing required fields, reserved-name collision, wrong `author`/`metadata` shape, out-of-root paths. Warnings (exit 0) are polish items: missing optional metadata, legacy `path` field, duplicate names.

See [`references/marketplace-audit.md`](references/marketplace-audit.md) for full severity tables, fix recipes, and the complementary built-in validator.
